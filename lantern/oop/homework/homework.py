...
class Wall:
    ...
    * Implement method number_of_rolls_of_wallpaper which receives such parameters: roll_width_m, roll_length_m
    ...
    def number_of_rolls_of_wallpaper(self, roll_width_m, roll_length_m):
        number_lines = roll_length_m // self.height
        number_lines2 = self.width // roll_width_m
        return number_lines2 / number_lines
...
class Door:
    ...
    def door_price(self, material) -> object:
        if material == 'wood':
    ...
    * Implement method get_room_square that returns the square of our room
    ...
    def create_door(self, width, height):
        if width == 0 or height == 0:
            raise ValueError("Value must be not 0")
        if self.__door:
            raise ValueError("The house can not has two doors")
